...
class LLMGenerationManager:
    ...
    def _generate_with_gpu_padding(self, active_batch: DataProto) -> DataProto:
        ...
        # # 1) Filter non-empty samples
        # non_empty = [(i, seq) for i, seq in enumerate(active_batch) if seq.input_ids.nelement() > 0]
        # if non_empty:
        #     idxs, seqs = zip(*non_empty)
        #     padded = pad_to_max_length(seqs)
        #     outs = self.actor_rollout_wg.generate_sequences(padded)
        #     # 2) Merge results back to original positions
        #     padded_output = [default_output] * len(active_batch)
        #     for idx, out in zip(idxs, outs):
        #         padded_output[idx] = out
        # else:
        #     # When all empty, can fill with default or skip
        #     padded_output = [default_output] * len(active_batch)
    ...
    def run_llm_loop(self, gen_batch, initial_input_ids: torch.Tensor, data_sources) -> Tuple[Dict, Dict]:
        ...
        for step in range(self.config.max_turns):
            ...
            next_obs, dones, valid_action, is_search = self.execute_predictions(
                responses_str, self.tokenizer.pad_token, active_mask, data_sources, do_search=self.config.do_search
            )
        ...
        if active_mask.sum():
            ...
            _, dones, valid_action, is_search = self.execute_predictions(
                responses_str, self.tokenizer.pad_token, active_mask, data_sources, do_search=False
            )
    ...
    def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, data_sources=None, do_search=True) -> List[str]:
        """
        ...
        allowed_idx = [
            idx for idx, (a, active) in enumerate(zip(cur_actions, active_mask))
            if active and a == 'search' and self.search_count[idx] < max_search
        ]

        # Prepare queries and data sources for these idx
        ...
        # search_data_sources = [ds for action, ds in zip(cur_actions, data_sources) if action == 'search']
        if do_search:
            search_results = self.batch_search(search_queries, search_data_sources)
            print("Search queries:", len(search_queries))
            # print("Data sources:", len(search_data_sources))
            # print('Search results:', len(search_results))
            # print('Action sums:', sum([1 for action in cur_actions if action == 'search']))
            # assert len(search_results) == sum([1 for action in cur_actions if action == 'search'])
            assert len(search_results) == len(allowed_idx)
        else:
            # search_results = [''] * sum([1 for action in cur_actions if action == 'search'])
            search_results = [''] * len(allowed_idx)
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    if self.search_count[i] >= max_search:
                        ...
                        info = search_results.pop(0).strip()
    ...
    def batch_search(self, queries: List[str] = None, data_sources: List[str] = None) -> str:
        """
    ...
    def _passages2string(self, retrieval_result, data_source=None):
        ...
        max_tokens = 746  # Maximum tokens for the final string
        total_tokens = 0
        ...
        for doc in selected:
            ...
            if is_doc:
                entry = f"[[{doc_id}]]:{body}\n"
                tokens = self.tokenizer.encode(entry, add_special_tokens=False)
                if (remain := max_tokens - total_tokens) <= 0:
                    break
                if len(tokens) > remain:
                    entry = self.tokenizer.decode(tokens[:remain], clean_up_tokenization_spaces=False)
                pieces.append(entry)
                total_tokens += min(len(tokens), remain)
            else:
